...
- 🧰 **Skills 与 MCP**——一键启用/停用策展的 Skills 与 MCP 服务器；开关在**下一新会话**生效（当前运行中的会话不会热重载）。
...
- 📚 **文档知识库（RAG）**——上传 PDF/文档，解析并嵌入到按项目或用户隔离的可检索知识库；会话可进一步限定到所选知识库。引用内容保留给模型参考。（RAG 默认关闭；需设置 `RAG_ENABLED=true` 并部署 parser sidecar。）
